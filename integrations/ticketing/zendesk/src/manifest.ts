...
export const zendeskTicketingProviderManifestInput = {
  ...
  capabilities: [
    ...
    { capability: "send", providerObjects: [{ kind: "zendeskTicketComment", label: "Zendesk Ticket Comment" }], requiresCredential: true, sideEffect: true, exposesSensitiveData: true, changesWorkflow: true },
    { capability: "draft", providerObjects: [{ kind: "zendeskInternalNote", label: "Zendesk Internal Note" }], requiresCredential: true, sideEffect: true, exposesSensitiveData: true },
    { capability: "attach", providerObjects: [{ kind: "zendeskTicketAttachment", label: "Zendesk Ticket Attachment" }], requiresCredential: true, sideEffect: true, exposesSensitiveData: true, changesWorkflow: true },
  ...
  operations: [
    ...
    { alias: "ticket.attachments.add", capability: "attach", providerOperation: "POST /api/v2/uploads.json + PUT /api/v2/tickets/{ticket_id}.json", providerObject: "attachment", sideEffect: true, exposesSensitiveData: true, changesWorkflow: true },
