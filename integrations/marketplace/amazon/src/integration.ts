...
export interface AmazonMarketplaceIntegrationOptions extends AmazonMarketplaceClientOptions {
...
function clientFor(
  ...
  if (!credentials) {
    if (hasClientOptions(options)) return createAmazonMarketplaceClient(options);
    throw new Error("Amazon marketplace operation credentials are required when no client is provided.");
  }
  return createAmazonMarketplaceClient({ ...options, ...credentials });
}
...
export function createAmazonMarketplaceIntegrationOperationHandlers(
  options: AmazonMarketplaceIntegrationOptions = {},
): AmazonMarketplaceOperations {
  return {
    [amazonMarketplaceOperationAliases.refreshAccessToken]: async (input, context) =>
      clientFor(amazonInput(input), options, credentialsFor(context)).refreshAccessToken(),
    [amazonMarketplaceOperationAliases.createRestrictedDataToken]: async (
      input,
      context,
    ) => clientFor(amazonInput(input), options, credentialsFor(context)).createRestrictedDataToken(input as Parameters<AmazonMarketplaceClient["createRestrictedDataToken"]>[0]),
    [amazonMarketplaceOperationAliases.getOrders]: async (
      input,
      context,
    ) => clientFor(amazonInput(input), options, credentialsFor(context)).getOrders(isRecord(input) ? input : undefined),
    [amazonMarketplaceOperationAliases.getOrder]: async (
      input,
      context,
    ) => clientFor(amazonInput(input), options, credentialsFor(context)).getOrder(readRequiredString(input, "amazonOrderId")),
    [amazonMarketplaceOperationAliases.getOrderBuyerInfo]: async (
      input,
      context,
    ) => clientFor(amazonInput(input), options, credentialsFor(context)).getOrderBuyerInfo(readRequiredString(input, "amazonOrderId")),
    [amazonMarketplaceOperationAliases.getOrderItems]: async (
      input,
      context,
    ) => clientFor(amazonInput(input), options, credentialsFor(context)).getOrderItems(readRequiredString(input, "amazonOrderId"), amazonInput(input) as { nextToken?: string }),
    [amazonMarketplaceOperationAliases.getOrderItemsBuyerInfo]: async (
      input,
      context,
    ) => clientFor(amazonInput(input), options, credentialsFor(context)).getOrderItemsBuyerInfo(readRequiredString(input, "amazonOrderId"), amazonInput(input) as { nextToken?: string }),
    [amazonMarketplaceOperationAliases.searchOrdersV2026]: async (
      input,
      context,
    ) => clientFor(amazonInput(input), options, credentialsFor(context)).searchOrdersV2026(isRecord(input) ? input : undefined),
    [amazonMarketplaceOperationAliases.getOrderV2026]: async (
      input,
      context,
    ) => clientFor(amazonInput(input), options, credentialsFor(context)).getOrderV2026(readRequiredString(input, "amazonOrderId"), amazonInput(input) as { includedData?: string[] }),
    [amazonMarketplaceOperationAliases.getSolicitationActionsForOrder]: async (
      input,
      context,
    ) => clientFor(amazonInput(input), options, credentialsFor(context)).getSolicitationActionsForOrder(readRequiredString(input, "amazonOrderId"), readStringArray(input, "marketplaceIds")),
    [amazonMarketplaceOperationAliases.createProductReviewAndSellerFeedbackSolicitation]: async (
      input,
      context,
    ) => clientFor(amazonInput(input), options, credentialsFor(context)).createProductReviewAndSellerFeedbackSolicitation(readRequiredString(input, "amazonOrderId"), readStringArray(input, "marketplaceIds")),
    [amazonMarketplaceOperationAliases.getMarketplaceParticipations]: async (
      input,
      context,
    ) => clientFor(amazonInput(input), options, credentialsFor(context)).getMarketplaceParticipations(),
    [amazonMarketplaceOperationAliases.getDestinations]: async (
      input,
      context,
    ) => clientFor(amazonInput(input), options, credentialsFor(context)).getDestinations(),
    [amazonMarketplaceOperationAliases.getDestination]: async (
      input,
      context,
    ) => clientFor(amazonInput(input), options, credentialsFor(context)).getDestination(readRequiredString(input, "destinationId")),
    [amazonMarketplaceOperationAliases.createSubscription]: async (
      input,
      context,
    ) => clientFor(amazonInput(input), options, credentialsFor(context)).createSubscription(input as Parameters<AmazonMarketplaceClient["createSubscription"]>[0]),
    [amazonMarketplaceOperationAliases.getSubscription]: async (
      input,
      context,
    ) => clientFor(amazonInput(input), options, credentialsFor(context)).getSubscription(input as Parameters<AmazonMarketplaceClient["getSubscription"]>[0]),
    [amazonMarketplaceOperationAliases.deleteSubscription]: async (
      input,
      context,
    ) => clientFor(amazonInput(input), options, credentialsFor(context)).deleteSubscription(input as Parameters<AmazonMarketplaceClient["deleteSubscription"]>[0]),
...
export function createAmazonMarketplaceIntegration(options: AmazonMarketplaceIntegrationOptions = {}) {
  return defineIntegration({
    manifest: amazonMarketplaceProviderManifest,
    operations: createAmazonMarketplaceIntegrationOperationHandlers(options),
  });
}

function hasClientOptions(options: AmazonMarketplaceIntegrationOptions) {
  return Boolean(
    options.accessToken
...
export const amazonMarketplaceIntegration = createAmazonMarketplaceIntegration();
export const createAmazonIntegration = createAmazonMarketplaceIntegration;
export const createAmazonIntegrationOperationHandlers = createAmazonMarketplaceIntegrationOperationHandlers;
