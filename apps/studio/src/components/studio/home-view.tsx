      ...
      <div className="flex min-h-0 items-end gap-4 border-b border-slate-200">
        {data.map((entry) => (
          <div className="flex min-w-0 flex-1 flex-col items-center justify-end gap-2" key={entry.journey}>
            ...
            <span className="block w-9 rounded-t bg-teal-700" style={{ height: `${Math.max(8, (entry.events / max) * chartHeight)}px` }} title={`${entry.events} events across ${entry.conversations} conversations`} />
          </div>
        ))}
      </div>
