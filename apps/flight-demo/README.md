...
The flight demo can sync one conversation between web chat, web voice, and Discord threads through `@cognidesk/integration-discord`. Discord support is disabled unless `discord.enabled` is true in `config.json`.
