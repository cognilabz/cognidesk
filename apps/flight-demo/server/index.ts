...
const studioAdapter = createCognideskStudioAdapter({
  ...
  runtime,
  conversations: {
    listConversations: (input) => runtime.listConversations(input),
    getConversation: (conversationId) => runtime.getConversation(conversationId),
  },
  configuration: {
    providerPackages,
    capabilityAvailability: [discordCapabilityAvailability, whatsAppCapabilityAvailability],
...
function flightDemoDiscordCapabilityAvailability(
  ...
  return CapabilityAvailabilitySchema.parse({
    ...
    enabledForChannels: externalApisEnabledForProvider ? ["chat", "voice"] : [],
    enabledForJourneys: externalApisEnabledForProvider ? [FLIGHT_HANDOFF_JOURNEY_ID] : [],
    blockers,
    metadata: {
      destination: "discord-support-thread",
      journeyEnabled: externalApisEnabledForProvider,
      providerEnabled: config.discord?.enabled === true,
...
function flightDemoWhatsAppCapabilityAvailability(
  ...
  return CapabilityAvailabilitySchema.parse({
    ...
    capability: "send",
    status: blockers.length > 0 ? "blocked" : "enabled",
    enabledForChannels: externalApisEnabledForProvider ? ["chat", "voice", "messaging"] : [],
    enabledForJourneys: externalApisEnabledForProvider ? [FLIGHT_WHATSAPP_JOURNEY_ID] : [],
    enabledForTools: externalApisEnabledForProvider ? ["sendWhatsAppCustomerMessage"] : [],
    blockers,
    metadata: {
      destination: "whatsapp-customer-message",
      journeyEnabled: externalApisEnabledForProvider,
      providerEnabled: config.whatsapp.enabled,
