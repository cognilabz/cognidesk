...
| `ticketing/salesforce` | Direct Salesforce Service Cloud support helpers and checked inventory evidence. | maintained-library | `@cognidesk/ticketing-salesforce` | Use `jsforce` v3 as the maintained runtime library while keeping Cognidesk coverage bounded to Service Cloud Case support workflows and SDK-user-owned SOQL escape hatches. |
