...
| `ecommerce/shopify` | Direct Admin GraphQL helpers plus generated Admin GraphQL root-field inventory. | official-sdk | `@cognidesk/ecommerce-shopify` | Cohort A. Use `@shopify/shopify-api`; keep selected direct Admin GraphQL support slices only where the SDK leaves a justified gap. |
| `ecommerce/stripe` | Generated Stripe OpenAPI surface plus direct client, validation, and webhook helpers. | official-sdk | `@cognidesk/ecommerce-stripe` | Cohort A. Use `stripe`; remove generated full API clone as normalized operations and raw client escape hatch land. |
...
| `ticketing/freshdesk` | Split `@cognidesk/ticketing-freshdesk` package with direct Freshdesk v2 support helpers, shared-secret webhook parsing, and checked inventory evidence. | direct-http-support-slice | `@cognidesk/ticketing-freshdesk` | No viable official Freshdesk ticketing backend SDK verified. Keep selected Freshdesk ticket/contact/agent/group operations. No full Freshdesk API claim. |
| `ticketing/front` | Split `@cognidesk/ticketing-front` package with direct Core/Channel API support helpers. | direct-http-support-slice | `@cognidesk/ticketing-front` | No viable official backend JS SDK verified; deprecated `front-sdk` is not adopted. Old generated full-provider clone is not copied. |
| `ticketing/gorgias` | Split `@cognidesk/ticketing-gorgias` package with direct REST ticket/message/account support helpers. | direct-http-support-slice | `@cognidesk/ticketing-gorgias` | No viable official backend JS SDK verified. Old generated full-provider clone is not copied. |
| `ticketing/help-scout` | Split `@cognidesk/ticketing-help-scout` package with direct Inbox API support helpers. | direct-http-support-slice | `@cognidesk/ticketing-help-scout` | Official JavaScript SDK targets Help Scout Apps UI, not backend Inbox API. Keep selected Inbox API support operations and explicit gap inventory. |
...
| `ticketing/kustomer` | Split `@cognidesk/ticketing-kustomer` package with direct REST conversation/message/customer-draft support helpers. | direct-http-support-slice | `@cognidesk/ticketing-kustomer` | Kustomer app/chat SDKs are not backend REST ticketing clients. Old generated full-provider clone is not copied. |
...
| `ticketing/zendesk` | Split `@cognidesk/ticketing-zendesk` package with direct Zendesk Support ticket/comment/upload/user/org/webhook readiness support helpers. | direct-http-support-slice | `@cognidesk/ticketing-zendesk` | `node-zendesk` is documented but not officially supported; old generated full-provider clone is not copied. |
