...
| #23 Gmail reference | `integrations/email/gmail` publishes `@cognidesk/email-gmail`; the old aggregate Gmail runtime and generated full API files are removed. | Landed | Use Gmail as the first SDK-backed reference pattern for manifest-only imports, raw official clients, and adapter coverage. |
