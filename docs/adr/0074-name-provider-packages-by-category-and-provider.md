...
The restored decision uses separate packages named `@cognidesk/{provider-category}-{provider}`, such as `@cognidesk/voice-twilio`, `@cognidesk/email-gmail`, `@cognidesk/ticketing-zendesk`, `@cognidesk/workplace-slack`, and `@cognidesk/contact-center-genesys-cloud`, while SDK-owned runtime and adapter packages keep the existing flat package style.
