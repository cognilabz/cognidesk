...
export function createTikTokSocialClient(options: TikTokSocialClientOptions): TikTokSocialClient {
  const openApiBaseUrl = (options.openApiBaseUrl ?? "https://open.tiktokapis.com").replace(/\/+$/, "");
  ...
  return {
    async getUserInfo(fields = defaultUserFields) {
    ...
    async queryResearchVideoComments(input) {
    ...
    async listBusinessVideos(input = {}) {
    ...
    async listBusinessComments(input) {
      const businessId = input.businessId ?? options.businessId;
    ...
    async replyToBusinessComment(input) {
      return tiktokBusinessRequest<TikTokApiResponse>({
    ...
    async createBusinessComment(input) {
      const businessId = input.businessId ?? options.businessId;
