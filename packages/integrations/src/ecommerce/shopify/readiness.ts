...
import type { ShopifyCredentialStatusInput, ShopifyLiveCheckOptions } from "./contracts.js";
import { shopifyEcommerceProviderManifest } from "./manifest.js";

export function shopifyEcommerceCredentialStatuses(
  ...
  return [
    {
      ...
      message: input.shopDomain
        ? `Shopify shop domain is configured${input.apiVersion ? ` for Admin API ${input.apiVersion}` : ""}.`
        : "A Shopify shop domain is required.",
    },
    {
      providerPackageId: shopifyEcommerceProviderManifest.id,
...
export function createShopifyEcommerceLiveChecks(options: ShopifyLiveCheckOptions) {
  return [{
    id: "shop",
    ...
    async run(context: { signal?: AbortSignal }) {
      const client = options.client ?? createShopifyEcommerceClient(options);
