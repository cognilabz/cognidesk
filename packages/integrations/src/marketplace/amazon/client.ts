...
export function createAmazonMarketplaceClient(options: AmazonMarketplaceClientOptions): AmazonMarketplaceClient {
  ...
  const request = <T = AmazonResource>(
    method: AmazonSpApiMethod,
    path: string,
    input: AmazonOperationRequestInput = {},
  ) => {
    const requestInput: AmazonRequestInit & {
      url: string;
      fetch: typeof fetch;
      options: AmazonMarketplaceClientOptions;
    } = {
      url: amazonOperationUrl(apiBaseUrl, path, input.pathParams),
      fetch: fetchImpl,
      options,
      method,
    };
    if (input.body !== undefined) requestInput.body = input.body;
    if (input.headers !== undefined) requestInput.headers = input.headers;
    if (input.query !== undefined) requestInput.query = input.query;
    return amazonRequest<T>(requestInput);
  };
  const requestOperation = async (
    operationUidOrId: AmazonFullApiOperationUid | string,
    input: AmazonOperationRequestInput = {},
  ) => {
    const operation = AMAZON_FULL_API_OPERATION_BY_UID.get(operationUidOrId)
      ?? AMAZON_FULL_API_OPERATION_BY_ID.get(operationUidOrId);
    if (!operation) throw new Error(`Unknown Amazon SP-API operation '${operationUidOrId}'.`);
    return request(operation.method, operation.path, input);
  };
  const fullApi = createAmazonFullApiGeneratedClient(requestOperation as AmazonGeneratedOperationCaller);
  ...
  return {
    fullApi,
    requestOperation: requestOperation as AmazonGeneratedOperationCaller,
