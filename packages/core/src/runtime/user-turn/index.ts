...
import type { HandleUserMessageDeps } from "./types.js";
...
export async function handleUserMessage<TTurn>(
  ...
  const turnChannel = args.input.channel
    ? defineChannelContext(args.input.channel)
  ...
  try {
    ...
    if (turnChannel) {
      await emit({
        ...
        data: {
          channel: turnChannel,
          text: userText,
          ...(args.input.turn !== undefined ? { payload: args.input.turn } : {}),
        },
      });
    }

    const app = args.input.app ?? args.options.app ?? {};
    const turnPayload = (args.input.turn ?? {}) as TTurn;
    const conversationContext = await redactConversationContextForPrompt(args.options, conversation);
    const resolvedContext = await resolveRuntimeApplicationContext({
      options: args.options,
      conversation,
      turn: turnPayload,
      ...(turnChannel ? { channel: turnChannel } : {}),
      app,
      text: userText,
    });
    throwIfTurnInterrupted(turn);
    if (args.options.context?.resolve) {
      await emit({
        conversationId: conversation.id,
        type: "context.resolved",
        data: {
          keys: topLevelContextKeys(resolvedContext),
          ...(turnChannel ? { channel: turnChannel } : {}),
        },
      });
    }
    const policyContext = resolveTurnPolicyContext({
      agent,
      ...(turnChannel ? { channel: turnChannel } : {}),
      ...(args.options.channels ? { runtimeChannels: args.options.channels } : {}),
    });
    if (turnChannel || policyContext.agentChannelPolicy !== undefined || policyContext.runtimeChannelPolicy) {
      await emit({
        conversationId: conversation.id,
        type: "policy.evaluated",
        data: {
          ...(turnChannel ? { channel: turnChannel } : {}),
          ...policyContextForEvent(policyContext),
        },
      });
    }
    ...
    const response = await args.generateResponseWithTools({
      ...
      promptPayload: buildModelVisiblePromptPayload({
        agent,
        messages: modelMessages,
        modelTools,
        selectedJourneyId: selectedJourney?.id ?? null,
    ...
    let assistantText = removeRawKnowledgeMarkers(
      await args.redactAssistantMessage(conversation, response.text),
      knowledge,
    );
    const limitedResponse = applyConfiguredResponseLimits(assistantText, policyContext.runtimeChannelPolicy);
    assistantText = limitedResponse.text;
    const policyApplied = Object.keys(limitedResponse.constraints).length > 0 || limitedResponse.truncated
      ? { ...limitedResponse.constraints, truncated: limitedResponse.truncated }
      : undefined;
    if (turnChannel || policyApplied || response.usage || args.options.context?.resolve) {
      await emit({
        conversationId: conversation.id,
    ...
    if (turnChannel && recordAssistantMessage) {
      await emit({
        conversationId: conversation.id,
        type: "channel.sent",
        data: {
