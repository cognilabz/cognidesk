...
export async function startRuntimeVoiceSegment(
  ...
  } satisfies RuntimeEventInput<"voice.segment.started">;
  const event = await appendVoiceSegmentStarted(options, emit, eventInput);
  if (!event) throw activeVoiceSegmentError(conversation.id);
...
export async function assertNoActiveVoiceSegment(options: RuntimeOptions, conversationId: string) {
  ...
  if (active.size > 0) {
    throw activeVoiceSegmentError(conversationId);
  }
}
...
async function appendVoiceSegmentStarted(
  options: RuntimeOptions,
  emit: RuntimeEventEmitter,
  event: RuntimeEventInput<"voice.segment.started">,
) {
  if (options.storage.appendEventIfNoActiveVoiceSegment) {
    const telemetry = activeRuntimeEventTelemetry(options);
    const stored = await options.storage.appendEventIfNoActiveVoiceSegment({
      ...event,
      ...(telemetry ? { telemetry } : {}),
    });
    if (stored) recordStoredRuntimeEvent(options, stored);
    return stored;
  }

  await assertNoActiveVoiceSegment(options, event.conversationId);
  return emit(event);
}

function activeVoiceSegmentError(conversationId: string) {
  return new Error(`Conversation '${conversationId}' already has an active voice segment.`);
}
