...
| `metadata?` | `Record`\<`string`, `unknown`\> |
| `operationAliases` | ( \| `"email.receive"` \| `"email.send"` \| `"email.reply.send"` \| `"email.deliveryStatus.read"` \| `"mailgun.storedMessage.read"` \| `"mailgun.domain.read"` \| `"mailgun.webhooks.list"`)[] |
