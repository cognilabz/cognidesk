...
| [`@cognidesk/integrations`](integrations.md) | External Provider Integrations with category/provider subpath imports |
...
Provider Integrations for email, messaging, SMS, workplace, social, ticketing, contact center, reviews, marketplace, video, voice provider APIs, and related support surfaces ship in `@cognidesk/integrations` and are imported through category/provider subpaths. OpenAI Realtime voice is available as `@cognidesk/integrations/voice/openai`. They are documented by capability in the [Provider Integrations guide](../guides/provider-packages.md).
