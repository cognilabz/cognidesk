...
- #23: Gmail reference package, `@cognidesk/integration-email-gmail`, official `@googleapis/gmail`.
- #24: Microsoft Graph Outlook/Teams packages, `@cognidesk/integration-email-outlook` and `@cognidesk/integration-workplace-teams`.
- #25: Slack/Discord packages, `@cognidesk/integration-workplace-slack` and `@cognidesk/integration-community-discord`.
...
- Publish as `@cognidesk/integration-{category}-{provider}`.
...
- #23 `codex/integrations-23-gmail-sdk-package`: `@cognidesk/integration-email-gmail` exists under `integrations/email/gmail` using `@googleapis/gmail`; package tests, architecture, conformance, catalog data/docs, codemod check, legacy aggregate build, and package smoke/size budgets passed.
- #24 `codex/integrations-24-microsoft-graph-packages`: `@cognidesk/integration-email-outlook` and `@cognidesk/integration-workplace-teams` exist using `@microsoft/microsoft-graph-client`; Outlook/Teams tests, architecture, conformance, catalog data/docs, codemod check, legacy aggregate build, and package smoke/size budgets passed.
- #25 `codex/integrations-25-chat-provider-packages`: `@cognidesk/integration-workplace-slack` uses `@slack/web-api`; `@cognidesk/integration-community-discord` uses `discord.js`; Slack/Discord tests, architecture, conformance, catalog data/docs, codemod check, legacy aggregate build, and package smoke/size budgets passed after fixing `packages/integrations` workplace build output to `dist/workplace/teams`.
- #25 follow-up after checking the latest `origin/main` merge `940d98b [codex] Fix flight demo Discord and voice handoff (#18)`: the branch is now pushed at `96a40ac feat(integrations): fold discord gateway into provider package`. The new Discord Gateway flight-demo package from main was folded into `integrations/community/discord` instead of preserving a second `@cognidesk/integration-discord` package. Keep the ChatGPT/ADR naming rule: npm package `@cognidesk/integration-community-discord`; visible integration/catalog names such as `Discord Integration`, `createDiscordIntegration`, and `discordIntegrationManifest`.
- #25 follow-up verification passed: browser recheck of the ChatGPT plan naming examples, `pnpm --filter @cognidesk/integration-community-discord test`, catalog data/docs generation, `node --check scripts/generate-provider-integration-catalog.mjs`, architecture/conformance checks, scoped old-import codemod check, Flight Demo typecheck/test, release package smoke/size budget, and `git diff --check`/`git diff --cached --check`. A full old-import codemod over `apps/flight-demo` still flags voice imports from `@cognidesk/integrations/voice/*`; leave those for #40/#41.
...
- The branch adds `@cognidesk/integration-email-mailgun`, `@cognidesk/integration-email-postmark`, `@cognidesk/integration-email-ses`, and `@cognidesk/integration-email-imap` under `integrations/email/*`.
...
- The branch adds `@cognidesk/integration-ecommerce-stripe` and `@cognidesk/integration-ecommerce-shopify` under `integrations/ecommerce/*`.
...
- The branch adds staged `@cognidesk/integration-marketplace-amazon` and `@cognidesk/integration-marketplace-ebay` packages under `integrations/marketplace/*`.
...
- The branch adds `@cognidesk/integration-messaging-whatsapp`, `@cognidesk/integration-social-instagram`, and `@cognidesk/integration-social-messenger` under `integrations/*/*`.
...
- The branch adds `@cognidesk/integration-messaging-rcs` and `@cognidesk/integration-social-tiktok` under `integrations/*/*`.
...
- The branch adds `@cognidesk/integration-review-googleplay` and `@cognidesk/integration-review-appstore` under `integrations/review/*`.
...
- The branch adds `@cognidesk/integration-ticketing-zendesk`, `@cognidesk/integration-ticketing-freshdesk`, `@cognidesk/integration-ticketing-front`, `@cognidesk/integration-ticketing-gorgias`, `@cognidesk/integration-ticketing-help-scout`, and `@cognidesk/integration-ticketing-kustomer` under `integrations/ticketing/*`.
...
- The branch adds `@cognidesk/integration-ticketing-hubspot`, `@cognidesk/integration-ticketing-intercom`, `@cognidesk/integration-ticketing-salesforce`, `@cognidesk/integration-ticketing-dynamics365`, `@cognidesk/integration-ticketing-servicenow`, and `@cognidesk/integration-ticketing-zoho-desk` under `integrations/ticketing/*`.
...
- The branch adds `@cognidesk/integration-ticketing-oracle-service`, `@cognidesk/integration-ticketing-pega-customer-service`, and `@cognidesk/integration-ticketing-sap-service-cloud` under `integrations/ticketing/*`.
...
- The branch adds `@cognidesk/integration-contact-center-amazon-connect`, `@cognidesk/integration-contact-center-genesys-cloud`, and `@cognidesk/integration-contact-center-ringcentral` under `integrations/contact-center/*`.
...
- Package naming fixtures were tightened away from stale non-category-prefixed examples. Provider packages must use `@cognidesk/integration-{category}-{provider}`; `@cognidesk/integration-kit` and `@cognidesk/integration-catalog` remain legitimate provider-neutral infrastructure packages.
...
- The branch adds nine split packages under `integrations/contact-center/*`: `@cognidesk/integration-contact-center-8x8`, `@cognidesk/integration-contact-center-aircall`, `@cognidesk/integration-contact-center-five9`, `@cognidesk/integration-contact-center-genesys-engage`, `@cognidesk/integration-contact-center-genesys-pureconnect`, `@cognidesk/integration-contact-center-nextiva`, `@cognidesk/integration-contact-center-nice-cxone`, `@cognidesk/integration-contact-center-talkdesk`, and `@cognidesk/integration-contact-center-zoom`.
...
- Package names follow the ChatGPT/ADR rule `@cognidesk/integration-{category}-{provider}` and no old-import bridge/shim/runtime resolver was added.
...
- The branch adds `@cognidesk/integration-voice-aws-speech`, `@cognidesk/integration-voice-azure-speech`, `@cognidesk/integration-voice-google-speech`, and the rehomed canonical `@cognidesk/integration-voice-openai` under `integrations/voice/*`.
- The ChatGPT package naming rule is applied explicitly: source paths are `integrations/{category}/{provider}` and public packages are `@cognidesk/integration-{category}-{provider}`. OpenAI Realtime voice is therefore `integrations/voice/openai` published as `@cognidesk/integration-voice-openai`, with visible manifest metadata such as `OpenAI Realtime Voice Integration`, `integrationPackageName`, and `/manifest` + `/runtime` entry points.
- `@cognidesk/integration-voice-openai` depends on `@cognidesk/voice-websocket` because OpenAI Realtime voice uses the shared Cognidesk browser voice transport. That dependency is package-to-package reuse, not a reason to make WebSocket a provider integration.
...
- Follow-up package-naming/browser recheck comment: https://github.com/cognilabz/cognidesk/issues/40#issuecomment-4762373256. The catalog docs now visibly show `Integration | OpenAI Realtime Voice Integration` next to `Package | @cognidesk/integration-voice-openai`.
...
- The branch adds final replacement packages `@cognidesk/integration-voice-deepgram`, `@cognidesk/integration-voice-elevenlabs`, `@cognidesk/integration-voice-twilio`, `@cognidesk/integration-sms-twilio`, and `@cognidesk/integration-voice-vonage` under `integrations/{category}/{provider}`.
...
- The branch adds final replacement packages `@cognidesk/integration-video-zoom` and `@cognidesk/integration-video-whereby` under `integrations/video/*`.
...
- The branch adds `@cognidesk/integration-cobrowsing-cognidesk`, `@cognidesk/integration-community-forum`, `@cognidesk/integration-form-cognidesk`, `@cognidesk/integration-help-center-cognidesk`, and `@cognidesk/integration-voice-sip` under `integrations/{category}/{provider}`.
...
- Package-naming fixtures were tightened again so provider examples use `@cognidesk/integration-{category}-{provider}` (`@cognidesk/integration-email-gmail`, `@cognidesk/integration-email-acme`, `@cognidesk/integration-sms-acme`) instead of stale non-category-prefixed names. `@cognidesk/integration-kit` and `@cognidesk/integration-catalog` remain legitimate provider-neutral infrastructure packages.
...
- Package naming follows the plan's category/provider shape with the explicit integration prefix requested after review: provider packages are `@cognidesk/integration-{category}-{provider}` such as `@cognidesk/integration-email-gmail`, `@cognidesk/integration-workplace-slack`, `@cognidesk/integration-ecommerce-stripe`, `@cognidesk/integration-sms-twilio`, and `@cognidesk/integration-voice-twilio`.
